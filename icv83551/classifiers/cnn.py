...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        fan_in_affine = num_filters * (H // 2) * (W // 2) 
        self.params['W2'] = weight_scale * np.random.randn(fan_in_affine, hidden_dim)
    ...
    def loss(self, X, y=None):
        ...
        loss += 0.5 * self.reg * (np.sum(W1**2) + np.sum(W2**2) + np.sum(W3**2)) # Add L2 regularization to loss
        
        dout, grads['W3'], grads['b3'] = affine_backward(dscores, cache3)
